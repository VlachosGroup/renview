...
C.....Ashish modification: IDIM increased for large gas mechs.
C
      PARAMETER (MDIM=50, KDIM=1000, MKDIM=MDIM*KDIM, IDIM=5000,LSYM=16,
...
C
      ...
      OPEN(LIN,FILE='INP.d/gas.inp',FORM='FORMATTED')
      OPEN(LOUT,FILE='OUT.d/gas.out',FORM='FORMATTED')
C----------------------------------------------------------------------C
C
      VERS = '3.0'
      WRITE  (LOUT, 15) VERS(:3)
   15 FORMAT (/
     1' CHEMKIN INTERPRETER OUTPUT: CHEMKIN-II Version ',A,' Jun. 1991'
C*****precision > double
     2/'                              DOUBLE PRECISION'/)
      PREC = 'DOUBLE'
C*****END precision > double
...
C*****OPEN statement > unix
            OPEN (LTHRM, FORM='FORMATTED', FILE='INP.d/thermdat')
