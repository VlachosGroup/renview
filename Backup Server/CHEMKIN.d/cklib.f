...
C
c        ashish insert
      open (linc, file='INP.d/cklink', form='unformatted')
...
c      write(*,*)'LENIWK,LENRWK,LENCWK',LENIWK,LENRWK,LENCWK
C
      REWIND LINC
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKQYPF  (P, T, Y, ICKWRK, RCKWRK, QF)
C
C  START PROLOGUE
C
C  SUBROUTINE CKQYPF  (P, T, Y, ICKWRK, RCKWRK, QF)
C     Returns the rates of progress for the reactions given pressure,
C     temperature and mass fractions;  see Eqs. (51) and (58).
C     This version computes the forward rates only.
C
C  INPUT
C     P      - Pressure.
C                   cgs units - dynes/cm**2
...
C
C  OUTPUT
C     QF     - Rates of progress for the reactions.
C                   cgs units - moles/(cm**3*sec)
C                   Data type - real array
C                   Dimension QF(*) at least II, the total number of
C                   reactions.
C
C  END PROLOGUE
C
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKRBEX (I, RCKWRK, RB)
C
C  START PROLOGUE
C
C  SUBROUTINE CKRAEX (I, RCKWRK, RB)*
C     Get/put the temperature exponent of the Ith reaction
C
C  INPUT
C     I      - Reaction number; I > 0 gets RB(I) from RCKWRK
C                               I < 0 puts RB(I) into RCKWRK
C                   Data type - integer scalar
C     RCKWRK - Array of real work space.
C                   Data type - real array
C                   Dimension RCKWRK(*) at least LENRWK.
C     If I < 1:
C     RB     - Temperature exponent for the Ith reaction.
C                   cgs units - none
C                   Data type - real scalar
C
C  OUTPUT
...
C  START PROLOGUE
C
C  SUBROUTINE CKREEX (I, RCKWRK, RE)*
C     Get/put the activation energy of the Ith reaction
C     Added by J.E. Sutton 2012/07/27
C
C  INPUT
C     I      - Reaction number; I > 0 gets RE(I) from RCKWRK
C                               I < 0 puts RE(I) into RCKWRK
C                   Data type - integer scalar
C     RCKWRK - Array of real work space.
C                   Data type - real array
C                   Dimension RCKWRK(*) at least LENRWK.
C     If I < 1:
C     RE     - Pre-exponential coefficient for the Ith reaction.
C                   cgs units - K
C                   Data type - real scalar
C
C  OUTPUT
C     If I > 1:
C     RE     - Pre-exponential coefficient for Ith reaction.
C                   cgs units - K
C                   Data type - real scalar
C
C  END PROLOGUE
...
C         This sets the activation energy to the maximum of 0, the free
C         energy of reaction, and the as-specified activation energy in
C         order to ensure that the activation energy is always
C         thermodynamically consistent with the reaction energy. Note
C         that this does not affect the more complicated rate expressions.
          EACT(I) = MAX(-SUMSMH,PAR(3,I),0.0D0)
   50 CONTINUE
C
      DO 20 I = 1, II
         CTB(I) = 1.0
         RKF(I) = PAR(1,I) * EXP(PAR(2,I)*ALOGT - EACT(I)/T)
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKREXS (K, RCKWRK, A7)
C
C  START PROLOGUE
C
C  SUBROUTINE CKREXS (K, RCKWRK, A7)
C
...
C
C  INPUT
C      K      - Integer species number; K>0 gets A7(*) from RCKWRK,
C                                       K<0 puts A7(*) into RCKWRK.
C                    Data type - integer scalar
C      RCKWRK - Array of real internal work space.
C                    Data type - real array
C
...
C               of temperature ranges used in fitting thermodynamic
C               properties.
C               Dimension A7(*) at least (MXTP-1), where MXTP is
C               the maximum number of temperatures used for fitting
C               the thermodynamic properties of the species.
C                    Data type - real array
C                    cgs units:  none
...
C
      DO 100 L = 1, MXTP-1
         NA7 = NCAA + (L-1)*NCP2 + (ABS(K)-1)*NCP2T + NCP1
         IF (K .GT. 0) THEN
            A7(L) = RCKWRK(NA7)
         ELSE
            RCKWRK(NA7) = A7(L)
...
C----------------------------------------------------------------------C
C
      SUBROUTINE CKBEP(ICKWRK,RCKWRK,T,nBEP,BEP_def,IBEP_rxn,EA_old)
C
...
C     BEP_def:  Real array containing slopes & intercepts of correlations of
C               length nBEP
C     IBEP_rxn: Integer array containing reaction directions and pointers to
C               correlation types of length NII, the number of reactions
...
C*****END precision > single
C
      DIMENSION ICKWRK(*), RCKWRK(*),BEP_def(nBEP,*),
     1      DELH(NII), IBEP_rxn(NII,*), EA_old(*),
     2      HORT(NKK), HIS(NII), HFS(NII), NUKI(NKK,NII)
C
      COMMON /CKSTRT/ NMM , NKK , NII , MXSP, MXTB, MXTP, NCP , NCP1,
     1                NCP2, NCP2T,NPAR, NLAR, NFAR, NLAN, NFAL, NREV,
     2                NTHB, NRLT, NWL,  IcMM, IcKK, IcNC, IcPH, IcCH,
     ...
     7                NcK2, NcK3, NcK4, NcI1, NcI2, NcI3, NcI4
C
C   Get H for all species and convert to K
      CALL CKHORT(T,ICKWRK,RCKWRK,HORT)
      HORT = HORT * T
C
C   Get Stoichiometric coefficients
      CALL CKNU(NKK,ICKWRK,RCKWRK,NUKI)
C
C   Now determine HIS, HFS, and DELH
      DO 15 Irxn=1,NII
         DELH(Irxn)=0.
         HIS(Irxn)=0.
         HFS(Irxn)=0.
         DO 20 Ispec=1,NKK
            NU=NUKI(Irxn,Ispec)
            IF (NU<0) THEN
               HIS(Irxn)=HIS(Irxn)+HORT(Ispec)*ABS(NU)
            ELSE
               HFS(Irxn)=HFS(Irxn)+HORT(Ispec)*NU
            END IF
            DELH(Irxn)=DELH(Irxn)+HORT(Ispec)*NU
C
   20 CONTINUE
   15 CONTINUE
C
...
C   If no correlation, skip rxn and use surf.inp
         IF ((IBEP_rxn(Irxn,1)) .NE. 0) THEN
C
            SLOPE=BEP_def(IBEP_rxn(Irxn,1),2)
            B=BEP_def(IBEP_rxn(Irxn,1),3)
C
            IF (INT(BEP_def(IBEP_rxn(Irxn,1),1)) .EQ. 0) THEN
...
C              This is proved in Sutton & Vlachos, ACS Catal. 2(8), 1624
               IF (INT(BEP_def(IBEP_rxn(Irxn,1),4))
     1            .NE. IBEP_rxn(Irxn,2)) THEN
                  SLOPE=1-SLOPE
               END IF
               Ea_new=SLOPE*DELH(Irxn)+B
            ELSE
...
C              definition of the IS & TS.
               IF (INT(BEP_def(IBEP_rxn(Irxn,1),1)) .EQ. 1) THEN
C                 FS TSS correlation
                  IF (INT(BEP_def(IBEP_rxn(Irxn,1),4))
     1               .EQ. IBEP_rxn(Irxn,2)) THEN
                     RIVAR=HFS(Irxn)
                  ELSE
                     RIVAR=HIS(Irxn)
                  END IF
